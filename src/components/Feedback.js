...
// Add a list of realistic customer names for healthcare products
const customerNames = [
  // Healthcare Professionals
  { name: 'Dr. Sarah Johnson', email: 'sarah.johnson@mayoclinic.org', company: 'Mayo Clinic', role: 'Anesthesiologist' },
  { name: 'Dr. Michael Chen', email: 'mchen@clevelandclinic.org', company: 'Cleveland Clinic', role: 'Critical Care Specialist' },
  { name: 'Nurse Emily Rodriguez', email: 'emily.r@stanfordhealth.org', company: 'Stanford Health', role: 'ICU Nurse' },
  { name: 'Dr. James Wilson', email: 'jwilson@massgeneral.org', company: 'Massachusetts General Hospital', role: 'Surgeon' },
  { name: 'Lisa Thompson', email: 'lthompson@kaiserpermanente.org', company: 'Kaiser Permanente', role: 'Pharmacy Manager' },
  { name: 'Dr. Robert Martinez', email: 'rmartinez@johnshopkins.org', company: 'Johns Hopkins Medicine', role: 'Nephrologist' },
  { name: 'Dr. Amanda Lee', email: 'alee@mdanderson.org', company: 'MD Anderson Cancer Center', role: 'Oncologist' },
  { name: 'David Brown', email: 'dbrown@clevelandclinic.org', company: 'Cleveland Clinic', role: 'Biomedical Engineer' },
  
  // Internal Stakeholders
  { name: 'Jennifer Adams', email: 'jennifer.adams@healthcare.com', company: 'Healthcare International', role: 'Product Manager' },
  { name: 'Dr. Mark Stevens', email: 'mark.stevens@healthcare.com', company: 'Healthcare Corp', role: 'Clinical Director' },
  { name: 'Sarah Williams', email: 'sarah.williams@healthcare.com', company: 'Healthcare International', role: 'Customer Success Manager' },
  { name: 'Dr. Robert Chen', email: 'robert.chen@healthcare.com', company: 'Healthcare Corp', role: 'Medical Affairs' },
  { name: 'Lisa Garcia', email: 'lisa.garcia@healthcare.com', company: 'Healthcare International', role: 'Quality Assurance' },
  { name: 'Michael Johnson', email: 'michael.johnson@healthcare.com', company: 'Healthcare Corp', role: 'Regulatory Affairs' },
  { name: 'Emily Davis', email: 'emily.davis@healthcare.com', company: 'Healthcare International', role: 'Marketing Manager' },
  { name: 'David Wilson', email: 'david.wilson@healthcare.com', company: 'Healthcare Corp', role: 'Sales Director' }
];

// Add a list of realistic feedback messages for Baxter products based on eCatalog
const feedbackMessages = [
  // Anesthesia Gases & Critical Care
  'Anesthesia gas delivery system showing inconsistent flow rates during procedures.',
  'Critical care drug administration pump experiencing calibration issues.',
  'Anesthesia monitoring system alerts are too frequent and disruptive.',
  'Gas flow control mechanism needs better precision for pediatric cases.',
  'Anesthesia workstation interface is not intuitive for new users.',
  
  // Access & Infusion Systems
  'IV access device causing discomfort during long-term therapy.',
  'Infusion pump alarm system needs volume adjustment options.',
  'Large Volume Pump (LVP) showing intermittent flow interruptions.',
  'Starling Fluid Management System requires better calibration instructions.',
  'Infusion system integration with hospital networks needs improvement.',
  
  // Advanced Surgery & Biologics
  'Surgical device sterilization process taking longer than expected.',
  'Biologic product packaging needs better tamper-evident features.',
  'Surgical combination products require clearer usage instructions.',
  'Advanced surgery device compatibility issues with existing equipment.',
  'Biologic storage and handling procedures need simplification.',
  
  // Pharmaceutical Products & IV Solutions
  'IV solution container flexibility issues during cold storage.',
  'Pharmaceutical product labeling needs larger font sizes.',
  'IV solution mixing process requires better contamination prevention.',
  'Drug reconstitution system needs more precise measurement capabilities.',
  'Pharmaceutical packaging needs better child-resistant features.',
  
  // Enteral & Oral Syringes
  'Enteral syringe plunger mechanism showing wear after repeated use.',
  'Oral medication delivery system needs better dose accuracy.',
  'Syringe barrel material causing allergic reactions in some patients.',
  'Enteral feeding tube connection system needs improvement.',
  'Oral syringe measurement markings need better visibility.',
  
  // IV Preparation & Compounding
  'Pharmacy workflow management system interface is too complex.',
  'Automated compounding system needs better error handling.',
  'IV preparation spike system requires easier insertion mechanism.',
  'Compounding workflow needs better contamination prevention.',
  'Pharmacy automation system integration with existing systems needs work.',
  
  // Acute Renal Therapy & Dialysis
  'Dialysis machine alarm system needs volume customization.',
  'Renal therapy monitoring system requires better data visualization.',
  'Dialysis catheter connection system needs improvement.',
  'Acute renal therapy device needs better portability features.',
  'Dialysis solution mixing process requires automation.',
  
  // Parenteral Nutrition & Peritoneal Dialysis
  'Parenteral nutrition solution stability issues during long-term storage.',
  'Nutrition additive mixing system needs better accuracy.',
  'Peritoneal dialysis catheter insertion process needs simplification.',
  'Nutrition solution packaging needs better portability.',
  'Dialysis solution temperature control system requires improvement.',
  
  // Patient Monitoring & Safety
  'Patient monitoring system alerts are not customizable enough.',
  'Safety features need better integration with hospital protocols.',
  'Monitoring device battery life is insufficient for long procedures.',
  'Patient data synchronization between devices needs improvement.',
  'Safety alarm system needs better differentiation between alert types.'
];
...
function Feedback() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const newFeedback = {
        ...
        aiTags: ['Anesthesia & Critical Care', 'Access & Infusion', 'Advanced Surgery', 'Pharmaceutical Products', 'Enteral & Oral', 'IV Preparation', 'Renal Therapy', 'Parenteral Nutrition', 'Patient Monitoring', 'Safety & Quality', 'Product Usability', 'Clinical Effectiveness', 'Device Integration', 'Patient Safety', 'Workflow Efficiency', 'Regulatory Compliance'],
  ...
  const topTags = Object.entries(tagCounts)
    .filter(([tag]) => ['Anesthesia & Critical Care', 'Access & Infusion', 'Advanced Surgery', 'Pharmaceutical Products', 'Enteral & Oral', 'IV Preparation', 'Renal Therapy', 'Parenteral Nutrition', 'Patient Monitoring', 'Safety & Quality', 'Product Usability', 'Clinical Effectiveness', 'Device Integration', 'Patient Safety', 'Workflow Efficiency', 'Regulatory Compliance'].includes(tag))
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
  ...
  // --- AI Highlights Data ---
  // Top Complaint Themes with descriptions for Baxter healthcare products
  const tagDescriptions = {
    'Anesthesia & Critical Care': 'Issues related to anesthesia gas delivery, monitoring systems, and critical care devices.',
    'Access & Infusion': 'Problems with IV access devices, infusion pumps, and fluid management systems.',
    'Advanced Surgery': 'Feedback on surgical devices, biologics, and combination products.',
    'Pharmaceutical Products': 'Issues with IV solutions, drug reconstitution, and pharmaceutical packaging.',
    'Enteral & Oral': 'Problems with enteral feeding, oral syringes, and medication delivery systems.',
    'IV Preparation': 'Issues with pharmacy workflow, compounding systems, and IV preparation.',
    'Renal Therapy': 'Feedback on dialysis machines, renal therapy devices, and catheter systems.',
    'Parenteral Nutrition': 'Problems with nutrition solutions, additives, and peritoneal dialysis.',
    'Patient Monitoring': 'Issues with monitoring systems, alarms, and patient data management.',
    'Safety & Quality': 'Concerns about product safety, quality control, and regulatory compliance.',
    'Product Usability': 'Feedback on user interface, ease of use, and device ergonomics.',
    'Clinical Effectiveness': 'Issues related to clinical outcomes, treatment efficacy, and patient results.',
    'Device Integration': 'Problems with system compatibility, network integration, and interoperability.',
    'Patient Safety': 'Concerns about patient safety features, alarms, and risk management.',
    'Workflow Efficiency': 'Issues affecting clinical workflow, process optimization, and time management.',
    'Regulatory Compliance': 'Problems with regulatory requirements, documentation, and compliance standards.'
  };
  ...
  const aiSmartData = React.useMemo(() => {
    ...
    const actions = [
      'Prioritize infusion pump alarm system improvements for critical care units.',
      'Launch a patient safety feature enhancement for anesthesia monitoring systems.',
      'Increase clinical support team coverage for renal therapy device issues.'
    ];
    ...
    // 4. Predicted Impact
    const predImpact = actions.length > 0 ? 'If no action is taken, patient safety incidents may increase by 15% and clinical workflow efficiency may decrease by 20% next month.' : 'No major impact predicted.';
  ...
  const aiActionRationales = [
    'Infusion pump alarm issues have increased by 25% this month, affecting patient safety in critical care.',
    'Anesthesia monitoring system complaints are causing increased clinical support calls.',
    'Renal therapy device integration problems are leading to longer setup times.'
  ];
  ...
  const aiConfidence = 'AI Confidence: 92% (based on clinical feedback trends and patient safety data)';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(activeView === 'latest' || activeView === 'prioritized') && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center border-t-4 border-blue-500">
              ...
              <span className="text-gray-600 mt-1">Total Healthcare Product Feedbacks</span>
            </div>
            <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center border-t-4 border-green-500">
              ...
              <span className="text-gray-600 mt-1">Today's Healthcare Feedbacks</span>
            </div>
            <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center border-t-4 border-red-500">
              ...
              <span className="text-gray-600 mt-1">Last 2 Weeks Healthcare</span>
            </div>
          </div>
        )}
        ...
        {activeView === 'latest' && (
          <div className="space-y-10 animate-fade-in">
            {/* Product Line Feedback Section */}
            <div>
              <div className="text-xl font-bold text-gray-800 mb-6">Product Line Feedback Overview</div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl shadow-lg p-6 border border-blue-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Anesthesia & Critical Care</h3>
                      <p className="text-sm text-gray-600">Gas delivery, monitoring, critical care devices</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {feedbacks.filter(f => f.aiTags.includes('Anesthesia & Critical Care')).slice(0, 3).map((feedback, index) => (
                      <div key={index} className="bg-white/70 rounded-lg p-3 border border-blue-200">
                        <div className="flex items-start gap-2">
                          <div className={`w-2 h-2 rounded-full mt-2 ${feedback.sentiment === 'positive' ? 'bg-green-500' : feedback.sentiment === 'negative' ? 'bg-red-500' : 'bg-yellow-500'}`}></div>
                          <div className="flex-1">
                            <p className="text-sm text-gray-700 line-clamp-2">{feedback.message}</p>
                            <div className="flex items-center gap-2 mt-2">
                              <span className="text-xs text-gray-500">{feedback.customer.name}</span>
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-2xl shadow-lg p-6 border border-green-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Access & Infusion</h3>
                      <p className="text-sm text-gray-600">IV access, pumps, fluid management</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {feedbacks.filter(f => f.aiTags.includes('Access & Infusion')).slice(0, 3).map((feedback, index) => (
                ...
                <div className="bg-gradient-to-br from-purple-50 to-violet-100 rounded-2xl shadow-lg p-6 border border-purple-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Advanced Surgery</h3>
                      <p className="text-sm text-gray-600">Biologics, devices, combination products</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {feedbacks.filter(f => f.aiTags.includes('Advanced Surgery')).slice(0, 3).map((feedback, index) => (
                ...
                <div className="bg-gradient-to-br from-orange-50 to-amber-100 rounded-2xl shadow-lg p-6 border border-orange-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Pharmaceutical Products</h3>
                      <p className="text-sm text-gray-600">IV solutions, drug reconstitution, packaging</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {feedbacks.filter(f => f.aiTags.includes('Pharmaceutical Products')).slice(0, 3).map((feedback, index) => (
                      <div key={index} className="bg-white/70 rounded-lg p-3 border border-orange-200">
                        <div className="flex items-start gap-2">
                          <div className={`w-2 h-2 rounded-full mt-2 ${feedback.sentiment === 'positive' ? 'bg-green-500' : feedback.sentiment === 'negative' ? 'bg-red-500' : 'bg-yellow-500'}`}></div>
                          <div className="flex-1">
                            <p className="text-sm text-gray-700 line-clamp-2">{feedback.message}</p>
                ...
                {/* Renal Therapy */}
                <div className="bg-gradient-to-br from-red-50 to-pink-100 rounded-2xl shadow-lg p-6 border border-red-200">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 bg-red-500 rounded-xl flex items-center justify-center">
                      <span className="text-white font-bold text-lg">R</span>
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Renal Therapy</h3>
                      <p className="text-sm text-gray-600">Dialysis machines, renal devices, catheters</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {feedbacks.filter(f => f.aiTags.includes('Renal Therapy')).slice(0, 3).map((feedback, index) => (
                ...
                <div className="bg-gradient-to-br from-teal-50 to-cyan-100 rounded-2xl shadow-lg p-6 border border-teal-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Patient Monitoring</h3>
                      <p className="text-sm text-gray-600">Monitoring systems, alarms, data management</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {feedbacks.filter(f => f.aiTags.includes('Patient Monitoring')).slice(0, 3).map((feedback, index) => (
                      <div key={index} className="bg-white/70 rounded-lg p-3 border border-teal-200">
                        <div className="flex items-start gap-2">
                          <div className={`w-2 h-2 rounded-full mt-2 ${feedback.sentiment === 'positive' ? 'bg-green-500' : feedback.sentiment === 'negative' ? 'bg-red-500' : 'bg-yellow-500'}`}></div>
                          <div className="flex-1">
                            <p className="text-sm text-gray-700 line-clamp-2">{feedback.message}</p>
            ...
            {/* Today's Feedback */}
            <div>
              <div className="text-xl font-bold text-gray-800 mb-4">Today's Healthcare Product Feedback</div>
            ...
            <div>
              <div className="text-xl font-bold text-gray-800 mb-4">Last 2 Weeks - Healthcare Products</div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-10 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => openQuickActionModal('task')} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-green-400 to-green-600 text-white font-bold shadow hover:from-green-500 hover:to-green-700 transition">
                <TaskAltIcon /> Create Clinical Task
              </button>
              ...
              <button onClick={() => openQuickActionModal('escalate')} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-purple-400 to-purple-600 text-white font-bold shadow hover:from-purple-500 hover:to-purple-700 transition">
                <ArrowUpwardIcon /> Escalate to Clinical Team
              </button>
            </div>
                  ...
                  {!quickActionModal.success ? (
                    <div className="flex flex-col gap-6 p-8">
                      ...
                      {quickActionModal.type === 'task' && (
                        <>
                          <div className="flex items-center gap-2 mb-2"><TaskAltIcon className="text-green-500" /><span className="text-lg font-bold text-gray-800">Create Clinical Task</span></div>
                          <input className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-green-400 focus:outline-none transition" placeholder="Clinical Task Title" />
                          <textarea className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-green-400 focus:outline-none transition" rows={3} placeholder="Clinical Notes" />
                          ...
                          <button className="w-full py-3 rounded-lg bg-gradient-to-r from-green-500 to-green-700 text-white font-bold text-lg shadow hover:from-green-600 hover:to-green-800 transition" onClick={submitQuickAction}>Submit Clinical Task</button>
                        </>
                      )}
                      {quickActionModal.type === 'survey' && (
                        <>
                          <div className="flex items-center gap-2 mb-2"><SendIcon className="text-blue-500" /><span className="text-lg font-bold text-gray-800">Send Clinical Survey</span></div>
                          <select className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-blue-400 focus:outline-none transition">
                            <option>Clinical Effectiveness Survey</option>
                            <option>Patient Safety Feedback</option>
                            <option>Device Usability Survey</option>
                            <option>Clinical Workflow Assessment</option>
                          </select>
                          <textarea className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-blue-400 focus:outline-none transition" rows={3} placeholder="Clinical Message" />
                          <button className="w-full py-3 rounded-lg bg-gradient-to-r from-blue-500 to-blue-700 text-white font-bold text-lg shadow hover:from-blue-600 hover:to-blue-800 transition" onClick={submitQuickAction}>Send Clinical Survey</button>
                        </>
                      )}
                      {quickActionModal.type === 'escalate' && (
                        <>
                          <div className="flex items-center gap-2 mb-2"><ArrowUpwardIcon className="text-purple-500" /><span className="text-lg font-bold text-gray-800">Escalate to Clinical Team</span></div>
                          <select className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-purple-400 focus:outline-none transition">
                            <option>Patient Safety Issue</option>
                            <option>Clinical Device Problem</option>
                            <option>Regulatory Compliance Concern</option>
                            <option>Clinical Workflow Issue</option>
                          </select>
                          <textarea className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-purple-400 focus:outline-none transition" rows={3} placeholder="Clinical Context" />
                          <input className="rounded-lg border border-gray-300 p-3 text-gray-800 focus:ring-2 focus:ring-purple-400 focus:outline-none transition" placeholder="Clinical Team Contact (optional)" />
