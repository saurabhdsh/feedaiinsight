...
const routingMap = {
  high: 'Clinical Engineering',
  medium: 'Biomedical Services',
  low: 'Clinical Support',
};

// Dummy AI root cause and timeline generator for demo
function getRootCause(incident) {
  if (incident.priority === 'high') return 'Recent equipment update caused clinical workflow disruption.';
  if (incident.priority === 'medium') return 'Intermittent device connectivity detected.';
...
function getAIInsights(incident) {
  return [
    ...
    'Potential impact: Clinical workflow delays for 100+ users.'
  ];
}
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 max-w-xl">
              Automated detection, prioritization, and routing of incidents from healthcare product feedback using AI. Incidents are prioritized and routed to the appropriate clinical teams for rapid resolution.
            </p>
          </div>
          {/* Filters */}
          <div className="flex flex-wrap gap-4 items-center bg-white/80 rounded-xl shadow px-4 py-3">
            ...
            <select value={routingFilter} onChange={e => setRoutingFilter(e.target.value)} className="rounded-lg border border-gray-300 px-3 py-1 text-sm focus:ring-2 focus:ring-indigo-400">
              ...
              <option value="Biomedical Services">Biomedical Services</option>
